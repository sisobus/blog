...
* [BOJ 9328 열쇠](http://acmicpc.net/problem/9328)
...
![p93281](../images/p93281.png)<br>
![p93282](../images/p93282.png)<br>
![p93283](../images/p93283.png)
...
    if ( isLower(s[y][x]) ) {
        ok[s[y][x]-'a'] = true;
        for ( int i = 0 ; i < mp[s[y][x]-'a'+'A'].size() ; i++ )
            if ( !vis[mp[s[y][x]-'a'+'A'][i].first][mp[s[y][x]-'a'+'A'][i].second] )
                go(mp[s[y][x]-'a'+'A'][i].first,mp[s[y][x]-'a'+'A'][i].second);
        s[y][x] = '.';
    }
    if ( isUpper(s[y][x]) ) {
        if ( ok[s[y][x]-'A'] ) s[y][x] = '.';
...
    scanf("%d",&tc);
    while ( tc-- ) {
        for ( map<char,vector<ii> >::iterator it=mp.begin();it!=mp.end();it++ )
            it->second.clear();
        mp.clear();
        ans=0;
        memset(s,0,sizeof(s));
...
        start.clear();
        for ( int i = 0 ; i < h ; i++ ) {
            if ( s[i][0] != '*' ) start.push_back(ii(i,0));
            if ( s[i][w-1] != '*' ) start.push_back(ii(i,w-1));
        }
        for ( int i = 1 ; i < w-1 ; i++ ) {
            if ( s[0][i] != '*' ) start.push_back(ii(0,i));
            if ( s[h-1][i] != '*' ) start.push_back(ii(h-1,i));
        }
        for ( int i = 0 ; i < start.size() ; i++ )
            if ( !vis[start[i].first][start[i].second] )
                go(start[i].first,start[i].second);
        printf("%d\n",ans);
    }
    return 0;
}
